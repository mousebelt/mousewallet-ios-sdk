...
public class PathPaymentOperation:Operation {
    ...
    public let destAsset:StellarAsset
    ...
    ///
    public init(sourceAccount:StellarKeyPair? = nil, sendAsset:StellarAsset, sendMax:Decimal, destination:StellarKeyPair, destAsset:StellarAsset, destAmount:Decimal, path:[StellarAsset]) throws {
    ...
    ///
    public init(fromXDR:PathPaymentOperationXDR, sourceAccount:StellarKeyPair? = nil) {
        self.sendAsset = try! StellarAsset.fromXDR(assetXDR: fromXDR.sendAsset)
        self.sendMax = Operation.fromXDRAmount(fromXDR.sendMax)
        self.destination = StellarKeyPair(publicKey: fromXDR.destinationID)
        self.destAsset = try! StellarAsset.fromXDR(assetXDR: fromXDR.destinationAsset)
        self.destAmount = Operation.fromXDRAmount(fromXDR.destinationAmount)
        var path = [StellarAsset]()
        for asset in fromXDR.path {
            path.append(try! StellarAsset.fromXDR(assetXDR: asset))
        }
        self.path = path
        super.init(sourceAccount: sourceAccount)
    }
