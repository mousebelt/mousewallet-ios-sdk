...
public class StellarAsset
{
    ...
    public private(set) var issuer:StellarKeyPair?
    ...
    ///
    public init?(type:Int32, code:String? = nil, issuer:StellarKeyPair? = nil) {
        self.type = type
        switch self.type {
    ...
    ///
    public static func fromXDR(assetXDR:AssetXDR) throws -> StellarAsset {
        
        var result: StellarAsset?
        switch assetXDR {
            case .native:
                result = StellarAsset(type:AssetType.ASSET_TYPE_NATIVE)
            
            case .alphanum4 (let a4):
                let issuerKeyPair = StellarKeyPair (publicKey: a4.issuer, privateKey: nil)
                result = StellarAsset(type:AssetType.ASSET_TYPE_CREDIT_ALPHANUM4, code:assetXDR.assetCode, issuer:issuerKeyPair)
            
            case .alphanum12 (let a12):
                let issuerKeyPair = StellarKeyPair (publicKey: a12.issuer, privateKey: nil)
                result = StellarAsset(type:AssetType.ASSET_TYPE_CREDIT_ALPHANUM12, code:assetXDR.assetCode, issuer:issuerKeyPair)
        }
