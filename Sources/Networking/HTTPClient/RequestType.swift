...
extension RequestType {
    ...
    public func build() -> ResultCrypto<URLRequest> {
        ...
        guard var components = URLComponents(url: url, resolvingAgainstBaseURL: true) else {
            return .failure(NRLWalletSDKError.requestError(.invalidURL))
        }
    ...
    public func buildResponse(from data: Data?, response: URLResponse?, error: Error?) -> ResultCrypto<Response> {
        ...
        guard let data = data, let urlResponse = response as? HTTPURLResponse else {
            return .failure(NRLWalletSDKError.responseError(.noContentProvided))
        }
        
        guard (200..<300).contains(urlResponse.statusCode) else {
            return .failure(NRLWalletSDKError.responseError(.unacceptableStatusCode(urlResponse.statusCode)))
        }
