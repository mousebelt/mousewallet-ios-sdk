...
internal extension BatchElementType {
    ...
    internal func result(from object: Any) -> Result<Request.Response> {
        ...
        guard version == receivedVersion else {
            return .failure(NRLWalletSDKError.responseError(.jsonrpcError(.unsupportedVersion(receivedVersion))))
        }
        
        let receivedId = dictionary["id"] as? Int
        guard id == receivedId else {
            return .failure(NRLWalletSDKError.responseError(.jsonrpcError(.responseNotFound(requestId: id, object: dictionary))))
        }
        ...
        case (nil, let errorObject?):
            return .failure(NRLWalletSDKError.responseError(.jsonrpcError(JSONRPCError(errorObject: errorObject))))
            
        case (let resultObject?, nil):
            do {
                return .success(try request.response(from: resultObject))
            } catch {
                return .failure(NRLWalletSDKError.responseError(.jsonrpcError(.resultObjectParseError(error))))
            }
            
        default:
            return .failure(NRLWalletSDKError.responseError(.jsonrpcError(.missingBothResultAndError(dictionary))))
        }
    }
}
